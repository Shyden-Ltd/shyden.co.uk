...
- **Calculator input policy**: plain digits only (`/^\d+$/`); reject decimals / scientific notation / separators / signs; reject `0`; cap at `Number.MAX_SAFE_INTEGER`. Error strings are exact (see Task 2).
...
describe('calculateGlory — upper bound', () => {
  ...
  it('rejects MAX_SAFE_INTEGER + 1', () => {
    expect(calculateGlory('9007199254740992')).toEqual({ ok: false, error: ERRORS.tooLarge });
  });
});
...
export function calculateGlory(rawInput: string): GloryOutcome {
  ...
  if (gloryPoints > Number.MAX_SAFE_INTEGER) return { ok: false, error: ERRORS.tooLarge };
